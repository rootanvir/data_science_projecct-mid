...
fix_outliers <- function(data, colname) {
  ...
  if (n_out > 0) {
    ...
    message("Fixed ", n_out, " outliers in ", colname, " (capped to [", round(lower, 2), ", ", round(upper, 2), "])")
  } else {
...
orig <- names(df_clean)
cat_vars <- setdiff(orig[sapply(df_smote, function(x) is.factor(x) || is.character(x))], "previous_loan_defaults_on_file")
...
for (v in cat_vars) {
  ...
  if (length(d)) {
    ...
    lv <- sub(paste0("^", v, "_"), "", d)
    pretty_df[[v]] <- factor(lv[max.col(M, ties.method = "first")], levels = levels(df_smote[[v]]))
    pretty_df[d] <- NULL
  }
}
...
age_summary <- aggregate(person_age ~ loan_status, data = df, FUN = function(x) c(mean = mean(x), median = median(x), sd = sd(x), min = min(x), max = max(x)))
age_summary <- do.call(data.frame, age_summary)

income_summary <- aggregate(person_income ~ loan_status, data = df, FUN = function(x) c(mean = mean(x), median = median(x), sd = sd(x), min = min(x), max = max(x)))
